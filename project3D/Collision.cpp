	...
	namespace Collision {
		...
		namespace Test {
			...
			bool AABBPlane(const Body * _aabb, const Body * _plane, CollisionInfo & _info)
			{
				...
				// Distance from Plane
				float distance = glm::dot(pPlane->GetNormal(), (_aabb->GetPosition() - _plane->GetPosition()));
			...
			namespace Helper {
				...
				float AABBPoint_ClosestPoint_SquaredDist(const Body * _aabb, const glm::vec3 & _point)
				{
					...
					for (int axis = 0; axis < 3; axis++) {
						...
						{
							double val = (bmin[axis] - _point[axis]);
							out[axis] += val * val;
						}

						if (_point[axis] > bmax[axis])
						{
							double val = (_point[axis] - bmax[axis]);
							out[axis] += val * val;
						}
					}
