...
namespace aie {
	...
	void Application::run(const char* title, int width, int height, bool fullscreen) {
		...
		if (createWindow(title, width, height, fullscreen) &&
			...
			static const double fixedInterval = 1.0 / 60.0;
			double fixedDTPool = 0;
			...
			while (!m_gameOver) {
				...
				fixedDTPool += deltaTime;
				while (fixedDTPool >= fixedInterval) {
					fixedUpdate(fixedInterval);
					fixedDTPool -= fixedInterval;
				}
